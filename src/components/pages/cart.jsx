...
function Cart() {
  const {products, cartItems,currency,updQuantity,navigate} = useContext(ShopContext)
          ...
          <div key={i} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
            <div className='flex items-start gap-6'>
              ...
              <div>
                ...
                <input onChange={(e)=>e.target.value==="" || e.target.value === 0 ? null : updQuantity(item._id,item.size,Number(e.target.value))} className='border max-w-10 sm:max-w-20 px-1 sm:px-2 py-1' type='number' min={1} defaultValue={item.quantity}/>
                <img onClick={()=>{
                  updQuantity(item._id,item.size,0)
